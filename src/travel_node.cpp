...
int main(int argc, char **argv) {
    ...
    nh.param<float> ("/lidar/min_range"     , min_range_, 1.0);
    nh.param<float> ("/lidar/max_range"     , max_range_, 64.0);
    nh.param<int>   ("/lidar/vert_scan"     , vert_scan, 64);
    nh.param<int>   ("/lidar/horz_scan"     , horz_scan, 4500);
    nh.param<float> ("/lidar/min_vert_angle", min_vert_angle, -24.8);
    nh.param<float> ("/lidar/max_vert_angle", max_vert_angle, 2);
    ...
    nh.param<float>("/tgs/resolution"    , tgf_res, 8);
    nh.param<int>  ("/tgs/num_iter"     , num_iter, 3);
    nh.param<int>  ("/tgs/num_lpr"      , num_lpr, 5);
    nh.param<int>  ("/tgs/num_min_pts"  , num_min_pts, 10);
    nh.param<float>("/tgs/th_seeds"     , th_seeds, 0.5);
    nh.param<float>("/tgs/th_dist"      , th_dist, 0.125);
    nh.param<float>("/tgs/th_outlier"   , th_outlier, 0.3);
    nh.param<float>("/tgs/th_normal"    , th_normal, 0.940);
    nh.param<float>("/tgs/th_weight"    , th_weight, 200);
    nh.param<float>("/tgs/th_obstacle"  , th_obstacle, 1.0);
    nh.param<float>("/tgs/th_lcc_normal", th_lcc_normal_similarity , 0.1);
    nh.param<float>("/tgs/th_lcc_planar", th_lcc_planar_dist , 0.1);
    ...
    bool debug;
    nh.param<int>  ("/aos/downsample"           , downsample, 1);
    nh.param<float>("/aos/car_width"            , car_width, 1.7);
    nh.param<float>("/aos/car_length"           , car_length, 4.2);
    nh.param<float>("/aos/lidar_width_offset"   , lidar_width_offset, -0.27);
    nh.param<float>("/aos/lidar_length_offset"  , lidar_length_offset, 0.0);
    nh.param<float>("/aos/th_horz_merg"         , horz_merge_thres, 0.4);
    nh.param<float>("/aos/th_vert_merg"         , vert_merge_thres, 0.5);
    nh.param<int>  ("/aos/vert_scan_size"       , vert_scan_size, 3);
    nh.param<int>  ("/aos/horz_scan_size"       , horz_scan_size, 5);
    nh.param<int>  ("/aos/horz_skip_size"       , horz_skip_size, 5);
    nh.param<int>  ("/aos/horz_extension_size"  , horz_extension_size, 5);
    nh.param<int>  ("/aos/min_cluster_size"     , min_cluster_size, 10);
    nh.param<int>  ("/aos/max_cluster_size"     , max_cluster_size, 30000);
